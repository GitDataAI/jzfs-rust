...
use gitdata::rpc::git_core::PathRequest;
use gitdata::rpc::git_core::PublickeyRequest;
use gitdata::rpc::git_core::RepositoryAccess;
use gitdata::rpc::git_core::RepositoryStoragePosition;
use gitdata::rpc::git_core::TokenRequest;
...
impl CoreGitRpc {
    ...
    async fn connect() -> anyhow::Result<Self> {
        ...
        let rpc = rpc?;
        let core_git_rpc =
            RepRepositoryClient::connect(format!("http://{}", rpc.coregit_node()?.url())).await?;
        Ok(CoreGitRpc {
            client : core_git_rpc,
        })
    }
...
impl rpc::git_core::rep_repository_server::RepRepository for GitCoreRpc {
    async fn path(
        ...
        let path = match self
            .state
            .repo_owner_name(request.owner, request.repo)
            .await
        {
            Ok(path) => path,
